...
export interface MapLocation {
  zone: 'word_forest' | 'memory_castle' | 'vocabulary_village' | 'knowledge_kingdom';
...
export type AdventureGameType = 'flashcard_duel' | 'word_match_race' | 'letter_builder' | 'word_chase';
export type WordStatus = 'healthy' | 'forgotten' | 'rescue_priority' | 'rescued' | 'mastered';
export type MapZone = 'word_forest' | 'memory_castle' | 'vocabulary_village' | 'knowledge_kingdom';
export type BadgeType = 'memory_master' | 'word_saver' | 'vocabulary_champion' | 'speed_runner' | 'perfectionist';
...
export const determineWordStatus = (health: number): WordStatus => {
  ...
  if (health >= GAME_CONFIG.HEALTH_THRESHOLDS.RESCUE_PRIORITY) return 'rescue_priority';
...
export const calculateXPReward = (gameResult: RescueGameResult): number => {
  ...
  if (gameResult.time_taken < 30) { // Speed bonus for quick completion
