...
export const EnhancedVocabularyBuilder: React.FC<EnhancedVocabularyBuilderProps> = ({
  words,
  onWordMastered,
  onSessionComplete,
  ...
  const handleRating = (rating: 'easy' | 'medium' | 'hard') => {
    ...
    announceToScreenReader(`Rated as ${rating}. ${
      currentWordIndex < sessionWords.length - 1 ? 'Moving to next word.' : 'Session complete!'
    }`);
  ...
  const completeSession = () => {
    ...
    announceToScreenReader(`Session completed! You reviewed ${reviewedWords.length} words with ${Math.round(accuracy)}% completion rate.`);
  ...
  if (sessionWords.length === 0) {
    return (
      <Card className={`max-w-2xl mx-auto ${accessibilitySettings.highContrast ? 'bg-black text-white border-white' : ''}`}>
  ...
  return (
    <div className={`max-w-4xl mx-auto space-y-6 p-4 ${
      ...
      {showSettings && enableAccessibility && (
        <Card className={`${accessibilitySettings.highContrast ? 'bg-gray-900 text-white border-white' : ''}`}>
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  onChange={() => toggleAccessibilitySetting('keyboardShortcuts')}
                  className="w-4 h-4"
                />
            ...
            {accessibilitySettings.keyboardShortcuts && (
              <div className={`mt-4 p-4 rounded-lg ${
                accessibilitySettings.highContrast ? 'bg-gray-800' : 'bg-gray-50'
              }`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                  <div><kbd className="px-2 py-1 bg-gray-200 text-gray-800 rounded">Space</kbd> Show definition / Rate as medium</div>
                  <div><kbd className="px-2 py-1 bg-gray-200 text-gray-800 rounded">←</kbd> Rate as hard</div>
                  <div><kbd className="px-2 py-1 bg-gray-200 text-gray-800 rounded">→</kbd> Rate as easy</div>
                  <div><kbd className="px-2 py-1 bg-gray-200 text-gray-800 rounded">↑</kbd> Rate as medium</div>
                  <div><kbd className="px-2 py-1 bg-gray-200 text-gray-800 rounded">S</kbd> Skip word</div>
                  <div><kbd className="px-2 py-1 bg-gray-200 text-gray-800 rounded">P</kbd> Pronounce word</div>
                  <div><kbd className="px-2 py-1 bg-gray-200 text-gray-800 rounded">R</kbd> Return to word</div>
                  <div><kbd className="px-2 py-1 bg-gray-200 text-gray-800 rounded">Esc</kbd> Pause session</div>
                </div>
              </div>
            )}
          </CardContent>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              {isPaused && <Badge variant="outline" className="ml-2">Paused</Badge>}
            </CardTitle>
          ...
          <div className="flex items-center justify-between">
            <div className="flex gap-4">
              <Badge variant="outline" className={accessibilitySettings.largeText ? 'text-lg px-4 py-2' : ''}>
          ...
          {accessibilitySettings.showProgress && (
            <div>
              ...
              <Progress value={progress} className="h-2" aria-label={`Session progress: ${Math.round(progress)}%`} />
            </div>
          )}
        </CardHeader>
      </Card>
      ...
      {/* Pause Overlay */}
      {isPaused && (
        <Card className={`${accessibilitySettings.highContrast ? 'bg-gray-900 text-white border-white' : 'bg-gray-50'}`}>
          <CardContent className="p-8 text-center">
            <Pause className="w-16 h-16 mx-auto mb-4 text-gray-400" />
            <h3 className={`font-bold mb-2 ${accessibilitySettings.largeText ? 'text-2xl' : 'text-xl'}`}>
              Session Paused
            </h3>
            <p className={`text-gray-600 mb-4 ${accessibilitySettings.largeText ? 'text-lg' : ''}`}>
              Take a break! Resume when you're ready.
            </p>
            <Button onClick={() => setIsPaused(false)} className="min-h-[48px] px-6">
      ...
      {!isPaused && (
        <Card className={`relative overflow-hidden ${
          accessibilitySettings.highContrast ? 'bg-gray-900 text-white border-white' : ''
        }`}>
            ...
            {!showDefinition ? (
              ...
              <div className="text-center space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center justify-center gap-4 mb-4">
                    {currentWord.emoji && (
                      <span className={accessibilitySettings.largeText ? 'text-8xl' : 'text-6xl'}>
                        {currentWord.emoji}
                      </span>
                    )}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    <Button 
                      ...
                      onClick={() => {
                        setShowDefinition(true);
                        announceToScreenReader(`Showing definition for ${currentWord.word}`);
                      }}
                  ...
                  <p id="show-definition-hint" className={`text-xs opacity-75 ${
              ...
              <div className="space-y-8">
                <div className="text-center space-y-4">
                  <div className="flex items-center justify-center gap-4 mb-4">
                    {currentWord.emoji && (
                      <span className={accessibilitySettings.largeText ? 'text-6xl' : 'text-4xl'}>
                        {currentWord.emoji}
                      </span>
                    )}
                  ...
                  <div className={`rounded-lg p-6 space-y-4 ${
                    accessibilitySettings.highContrast ? 'bg-gray-800' : 'bg-slate-50'
                  }`}>
                    <div>
                ...
                <div className="text-center space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row justify-center gap-4">
                    ...
                    >
                      ...
                      <div className={`text-xs ${accessibilitySettings.largeText ? 'text-sm' : ''}`}>
                        Review tomorrow
                      </div>
                    </Button>
      ...
      <div className="sr-only">
        <div id="hard-rating-description">
          Rating a word as hard will schedule it for review tomorrow and decrease its mastery level.
        </div>
        <div id="medium-rating-description">
          Rating a word as medium will schedule it for review in a few days and moderately increase its mastery level.
        </div>
        <div id="easy-rating-description">
          Rating a word as easy will schedule it for review next week and significantly increase its mastery level.
        </div>
      </div>
