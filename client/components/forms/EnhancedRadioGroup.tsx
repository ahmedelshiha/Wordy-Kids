...
export interface EnhancedRadioGroupProps extends Omit<FormFieldProps, 'children'> {
...
const colorThemes = {
  jungle: {
    ...
    radio: 'border-jungle data-[state=checked]:bg-jungle data-[state=checked]:border-jungle',
    accent: 'text-jungle',
  },
  ocean: {
    selected: 'border-blue-500 bg-blue-50 text-blue-900',
    unselected: 'border-blue-200 hover:border-blue-300 hover:bg-blue-50',
    radio: 'border-blue-500 data-[state=checked]:bg-blue-500 data-[state=checked]:border-blue-500',
  ...
  space: {
    ...
    radio: 'border-purple-500 data-[state=checked]:bg-purple-500 data-[state=checked]:border-purple-500',
    accent: 'text-purple-600',
  },
  rainbow: {
    selected: 'border-pink-500 bg-gradient-to-r from-pink-50 to-purple-50 text-pink-900',
    unselected: 'border-pink-200 hover:border-pink-300 hover:bg-pink-50',
    radio: 'border-pink-500 data-[state=checked]:bg-pink-500 data-[state=checked]:border-pink-500',
    accent: 'text-pink-600',
  },
};

const difficultyIndicators = {
  easy: { emoji: '🌟', color: 'bg-green-100 text-green-700 border-green-300', points: 5 },
  medium: { emoji: '⚡', color: 'bg-yellow-100 text-yellow-700 border-yellow-300', points: 10 },
  hard: { emoji: '🏆', color: 'bg-red-100 text-red-700 border-red-300', points: 15 },
};
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {option.difficulty && (
                      <Badge
                        variant="outline"
                        className={cn('text-xs px-1.5 py-0.5', difficultyIndicators[option.difficulty].color)}
                      >
                        <span className="mr-1">{difficultyIndicators[option.difficulty].emoji}</span>
                        {option.difficulty}
                      </Badge>
                    )}
            ...
            {unlockSequentially && index > 0 && isDisabled && (
              <div className="absolute inset-0 bg-gray-900/20 rounded-lg flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-600">Complete previous first</p>
                </div>
              </div>
            )}
          ...
          >
            <Label htmlFor={optionId}>
              <div className={cn(
                ...
                <div>
                  {option.emoji && (
                    <div className={cn('mb-2', styles.emoji)}>{option.emoji}</div>
                  )}
                  <div className={styles.label}>{option.label}</div>
                  {option.description && (
                    <div className={cn('mt-1 text-gray-600', styles.description)}>
                      {option.description}
                    </div>
                  )}
                </div>
        ...
        return (
          ...
          >
            <RadioGroupItem
              ...
              className={cn(styles.radio, colors.radio)}
            />
            <Label htmlFor={optionId} className={cn('flex items-center gap-2 cursor-pointer', styles.label)}>
              ...
              <div className="flex items-center gap-1 ml-2">
                ...
                {option.difficulty && (
                  <Badge variant="outline" className={cn('text-xs', difficultyIndicators[option.difficulty].color)}>
                    {option.difficulty}
                  </Badge>
                )}
                {pointsSystem && option.points && (
        ...
        {showExplanations && selectedOption?.description && (
          ...
          >
            <div className="flex items-start gap-2">
              ...
              <div>
                <h4 className="font-medium text-blue-900 mb-1">About your choice:</h4>
