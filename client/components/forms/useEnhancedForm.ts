...
export const useEnhancedForm = (
  ...
  const [formState, setFormState] = useState<EnhancedFormState>(() => {
    const savedData = persistKey
      ? localStorage.getItem(`form_${persistKey}`)
      : null;
    const savedValues = savedData
      ? JSON.parse(savedData).values
      : initialValues;

    return {
      fields: Object.keys({ ...initialValues, ...savedValues }).reduce(
        (acc, key) => {
          acc[key] = {
  ...
  const errors = Object.keys(formState.fields).reduce(
    (acc, key) => {
  ...
  const setFieldValue = useCallback(
    (fieldName: string, value: any) => {
      setFormState((prev) => {
        ...
        if (showProgress) {
          const filledFields = Object.values(newFields).filter(
            (field) =>
              field.value !== null &&
              field.value !== undefined &&
              field.value !== "",
          ).length;
          newState.completionPercentage = Math.round(
            (filledFields / Object.keys(newFields).length) * 100,
          );
        }
      ...
      if (validationMode === "onChange" || validationMode === "all") {
        ...
        validationTimeouts.current[fieldName] = setTimeout(async () => {
          ...
          setFormState((prev) => {
            ...
            const hasErrors = Object.values(newFields).some(
              (field) => field.error,
            );
  ...
  const validateForm = useCallback(async (): Promise<FormValidationResult> => {
    const fieldResults: Record<
      string,
      { isValid: boolean; message: string; icon: string }
    > = {};
  ...
  const reset = useCallback(
    (newValues = initialValues) => {
      setFormState({
        fields: Object.keys(newValues).reduce(
          (acc, key) => {
            acc[key] = {
              value: newValues[key] ?? "",
              touched: false,
