...
export const FormField: React.FC<FormFieldProps> = ({
  ...
  const fieldState = error ? 'error' : success || isValid ? 'success' : 'default';
...
export const useFormField = (initialValue: string = '') => {
  ...
  const validate = React.useCallback(
    async (validationFn: (value: string) => Promise<{ isValid: boolean; message: string }> | { isValid: boolean; message: string }) => {
      if (!isTouched) return;
