...
import { PowerUpSystem, AchievementSystem, GameSessionManager } from '@/lib/enhancedGameplayMechanics';
...
export function EnhancedGameHub({
  ...
  // State management
  const [activeSection, setActiveSection] = useState<'hub' | 'enhanced-quiz' | 'word-garden' | 'adventure-games' | 'classic-quizzes' | 'settings'>('hub');
  ...
  const gameModes: GameMode[] = [
    {
      ...
      description: 'Premium AAA gaming experience with 3D effects, power-ups, and immersive jungle exploration',
    ...
    },
    {
      ...
      description: 'Listen and pick the matching picture in our beautiful jungle garden',
    ...
    },
    {
      ...
      description: 'Fast-paced vocabulary challenges with time pressure and streak multipliers',
    ...
    },
    {
      ...
      description: 'Relaxed gameplay without timers, perfect for mindful learning',
    ...
    },
    {
      ...
      description: 'Explore the jungle to find hidden word treasures with special rewards',
  ...
  const availableGameModes = gameModes.filter(mode => playerLevel >= mode.unlockLevel);
    ...
    return (
      <div className="min-h-screen">
        <EnhancedJungleQuizAdventure
          ...
          difficulty={gameSettings.difficulty}
          gameMode={selectedGameMode?.id === 'zen-mode' ? 'zen' : 
                   selectedGameMode?.id === 'speed-challenge' ? 'challenge' : 'adventure'}
        />
      </div>
    );
  }
  ...
  if (activeSection === 'word-garden') {
    return (
      <div className="space-y-6">
        ...
        <WordGarden
          selectedCategory={selectedCategory}
          onComplete={(stats) => onGameComplete(stats.score || 0, stats.totalWords || 10, stats)}
            ...
            <Card className="jungle-treasure-card border-4 border-yellow-400 relative overflow-hidden">
              ...
              <CardContent className="relative z-10">
                ...
                <Button
                  ...
                  {...createTouchAreaHandler(() => handleGameModeSelect(featuredMode))}
                >
            ...
            {availableGameModes.map((mode) => {
              ...
              return (
                <Card
                  ...
                  style={getTouchableStyle()}
                  {...(!isLocked ? createTouchAreaHandler(() => handleGameModeSelect(mode)) : {})}
                >
                  ...
                  {isLocked && (
                    <div className="absolute inset-0 bg-gray-900/50 flex items-center justify-center z-30">
                      <div className="text-center text-white">
                        ...
                        <p className="font-bold">Level {mode.unlockLevel} Required</p>
                      </div>
                    </div>
                  )}
                  ...
                  <CardHeader className="text-center relative z-10">
                    <div className="flex justify-center mb-3">
                      <div className={`p-3 rounded-full text-white shadow-md ${
                        mode.difficulty === 'easy' ? 'bg-green-500' :
                        mode.difficulty === 'medium' ? 'bg-yellow-500' :
                        'bg-red-500'
                      }`}>
                        {mode.icon}
                      </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card 
              ...
              style={getTouchableStyle()}
              {...createTouchAreaHandler(() => setActiveSection('adventure-games'))}
            >
            ...
            <Card 
              ...
              style={getTouchableStyle()}
              {...createTouchAreaHandler(() => setActiveSection('classic-quizzes'))}
            >
              <CardHeader className="text-center">
                <div className="flex justify-center mb-3">
