...
export function AdventureGames({ 
  ...
  const adventureGames = [
    ...
    },
    {
      ...
      difficulty: "Hard",
      timeLimit: "1-3 min",
      rewards: ["🚀 +30 points per word", "💨 Speed achievements", "👑 Speed champion"],
    ...
    },
    {
      ...
      difficulty: "Easy",
      timeLimit: "2-4 min",
      rewards: ["🔊 +15 points per word", "🎵 Sound master badge", "🗣️ Speaking confidence"],
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="bg-white/10 rounded-lg p-3">
                  ...
                  <div className="space-y-1">
                    {game.rewards.map((reward, i) => (
                      <div key={i} className="text-xs text-white/80">{reward}</div>
                    ))}
                  </div>
                </div>
              </div>
      ...
      <div className="text-center mt-8">
        <h3 className="text-lg font-bold text-gray-700 mb-4">🚀 Coming Soon!</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { name: "Story Builder", icon: "📚", desc: "Create stories with new words!" },
            { name: "Word Racing", icon: "🏎️", desc: "Race against time and friends!" },
            { name: "Treasure Hunt", icon: "🗺️", desc: "Find hidden word treasures!" }
          ].map((game, index) => (
