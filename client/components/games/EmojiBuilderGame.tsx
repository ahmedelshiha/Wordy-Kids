...
export const EmojiBuilderGame: React.FC<EmojiBuilderGameProps> = ({
  options,
  onSuccess,
  onFail,
}) => {
  ...
  return (
    <div className="flex flex-col items-center gap-4 text-center">
      ...
      <div className="flex gap-3">
        {parts.map((p, i) => (
          <button
            key={i}
            onClick={() => addPart(p)}
            className="w-[90px] h-[90px] text-5xl bg-white rounded-2xl shadow-lg border focus:outline-none focus:ring-2 focus:ring-blue-400"
            aria-label={`${p} part button`}
          >
            <span aria-hidden>{p}</span>
          </button>
        ))}
      </div>
      <div className="mt-2 text-6xl" aria-label="assembled emoji preview">
        {assembled.join(" ") || "_ _ _"}
      </div>
      <Button onClick={check} className="mt-2">
        Check ✅
      </Button>
      <div
      ...
      <GameRewardOverlay
        show={feedback !== null}
        correct={!!feedback}
        message={feedback ? "Nice! +💎" : "Try again"}
      />
    </div>
  );
};
