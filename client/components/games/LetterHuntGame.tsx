...
export const LetterHuntGame: React.FC<LetterHuntGameProps> = ({
  ...
  useEffect(() => {
    ...
    const arr = [firstLetter, ...others.slice(0, setSize - 1)].sort(
      () => Math.random() - 0.5,
    );
    setChoices(arr);
  }, [firstLetter, setSize]);
  ...
  return (
    <div className="flex flex-col items-center gap-4 text-center">
      ...
      <div className="grid grid-cols-3 gap-3">
        {choices.map((l, i) => (
          <button
            key={i}
            onClick={() => pick(l)}
            className="w-[90px] h-[90px] text-4xl bg-white rounded-2xl shadow-lg border focus:outline-none focus:ring-2 focus:ring-blue-400"
            aria-label={`Letter ${l}`}
          >
            {l}
          </button>
      ...
      <GameRewardOverlay
        show={feedback !== null}
        correct={!!feedback}
        message={feedback ? "Great! +💎" : "Try again"}
      />
    </div>
  );
};
