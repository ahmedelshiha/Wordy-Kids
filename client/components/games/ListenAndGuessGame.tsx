...
function speakWord(word: string, funny = false, fallback?: string) {
  ...
  if (synth && typeof SpeechSynthesisUtterance !== "undefined") {
    ...
    const pref = voices.find(v => /child|kid|female|english/i.test(v.name + " " + v.lang)) || voices[0];
...
export default function ListenAndGuessGame({
  ...
  const sequence = useMemo(() => {
    ...
    for (let i = 0; i < Math.min(rounds, pool.length); i++) {
      const w = pool[i];
      const distractors = pickN(w.distractorImages, Math.max(0, optionsPerRound - 1));
  ...
  const choose = useCallback((img: string) => {
    ...
    if (isCorrect) {
      ...
      setStreak(0);
      // gentle buzz
      if (navigator && "vibrate" in navigator) (navigator as any).vibrate([40, 60, 40]);
    }
    ...
    setTimeout(() => {
      ...
      if (roundIdx + 1 < sequence.length) setRoundIdx(r => r + 1);
        ...
        const stats: GameFinishStats = {
          ...
          streakBest: Math.max(bestStreak, isCorrect ? streak + 1 : bestStreak),
        };
        onFinish?.(stats);
      }
    }, 1500);
  }, [current, locked, roundIdx, sequence.length, correctCount, wrongCount, coins, bestStreak, streak, fire, onFinish]);
      ...
      <div className="rounded-3xl shadow-xl p-4 md:p-6 bg-gradient-to-b from-sky-600 to-blue-700 text-white">
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between text-sm opacity-90">
            <span>Round {roundIdx + 1} / {sequence.length}</span>
            <span>⭐ {coins}</span>
          </div>
          <div className="mt-2 h-3 rounded-full bg-white/20">
            <div className="h-3 rounded-full bg-white/80 transition-all duration-300" style={{ width: `${progressPct}%` }} />
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <div className="leading-tight">
              <div className="text-xs uppercase tracking-wide opacity-90">Listen & Guess</div>
        ...
        {/* Options grid */}
        <div className={`grid gap-3 ${optionsPerRound === 5 ? "grid-cols-3" : optionsPerRound === 4 ? "grid-cols-2" : "grid-cols-3"}`}>
          {current.options.map((img, i) => {
            ...
            return (
              ...
              >
                <img src={img} alt="option" className="w-full h-full object-contain p-2" />
                {/* fun corner badge */}
        ...
        <div className="mt-5">
          ...
          <div className="mt-2 h-3 rounded-full bg-white/20">
            <div className="h-3 rounded-full bg-emerald-300 transition-all duration-300" style={{ width: `${xpPct}%` }} />
          </div>
        </div>
      </div>
...
function EmptyState() {
  return (
    <div className="rounded-3xl bg-gradient-to-b from-slate-100 to-slate-50 p-6 text-center shadow-xl">
      ...
      <p className="text-slate-600">Provide words with images (and optional audio) to play the game.</p>
    </div>
  );
}
...
export const DemoWords: WordItem[] = [
  { id: 1, word: "star", imageUrl: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=200&h=200&fit=crop&auto=format", distractorImages: ["https://images.unsplash.com/photo-1583337130417-3346a1be7dee?w=200&h=200&fit=crop&auto=format","https://images.unsplash.com/photo-1464207687429-7505649dae38?w=200&h=200&fit=crop&auto=format"], audioUrl: undefined },
  { id: 2, word: "banana", imageUrl: "https://images.unsplash.com/photo-1571771894821-ce9b6c11b08e?w=200&h=200&fit=crop&auto=format", distractorImages: ["https://images.unsplash.com/photo-1574158622682-e40e69881006?w=200&h=200&fit=crop&auto=format","https://images.unsplash.com/photo-1494905998402-395d579af36f?w=200&h=200&fit=crop&auto=format"], audioUrl: undefined },
  { id: 3, word: "ball", imageUrl: "https://images.unsplash.com/photo-1593766827228-8737b4534aa6?w=200&h=200&fit=crop&auto=format", distractorImages: ["https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=200&h=200&fit=crop&auto=format","https://images.unsplash.com/photo-1544723795-3fb6469f5b39?w=200&h=200&fit=crop&auto=format"], audioUrl: undefined },
  { id: 4, word: "bird", imageUrl: "https://images.unsplash.com/photo-1552728089-57bdde30beb3?w=200&h=200&fit=crop&auto=format", distractorImages: ["https://images.unsplash.com/photo-1583212292454-1fe6229603b7?w=200&h=200&fit=crop&auto=format","https://images.unsplash.com/photo-1544551763-46a013bb70d5?w=200&h=200&fit=crop&auto=format"], audioUrl: undefined },
  { id: 5, word: "sun", imageUrl: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=200&h=200&fit=crop&auto=format", distractorImages: ["https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=200&h=200&fit=crop&auto=format","https://images.unsplash.com/photo-1574158622682-e40e69881006?w=200&h=200&fit=crop&auto=format"], audioUrl: undefined },
  { id: 6, word: "car", imageUrl: "https://images.unsplash.com/photo-1494905998402-395d579af36f?w=200&h=200&fit=crop&auto=format", distractorImages: ["https://images.unsplash.com/photo-1593766827228-8737b4534aa6?w=200&h=200&fit=crop&auto=format","https://images.unsplash.com/photo-1552728089-57bdde30beb3?w=200&h=200&fit=crop&auto=format"], audioUrl: undefined },
];
