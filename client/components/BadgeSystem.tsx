...
type BadgeCategory = 'rescue' | 'skill' | 'speed' | 'streak' | 'milestone' | 'progression' | 'wealth';
...
export const BadgeSystem: React.FC<BadgeSystemProps> = ({
  ...
  const calculateBadgeProgress = (badgeId: string) => {
    ...
    switch (badge.requirements.type) {
      ...
      case 'perfect_games':
        ...
        current = hero.badges.filter(b => b.id.includes('perfectionist')).length;
        break;
      case 'speed_record':
        // Would need to track this separately
        current = 0;
  ...
  const badgesByCategory = Object.values(BADGE_DEFINITIONS).reduce((acc, badge) => {
    if (!acc[badge.category]) {
      acc[badge.category] = [];
    }
    acc[badge.category].push(badge);
    return acc;
  }, {} as Record<BadgeCategory, typeof BADGE_DEFINITIONS[keyof typeof BADGE_DEFINITIONS][]>);
              ...
              <CardContent className="p-6 text-center">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    <span className={progress.earned ? 'text-green-600' : 'text-gray-500'}>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(badgesByCategory).map(([category, badges]) => {
              ...
              return (
                <div key={category} className="text-center p-4 bg-white rounded-lg">
                  <CategoryIcon className={`w-8 h-8 mx-auto mb-2 ${categoryInfo.color}`} />
                  <div className="font-bold text-lg">{earnedInCategory}/{badges.length}</div>
