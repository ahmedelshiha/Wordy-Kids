...
export function NavigationHistory({ 
  ...
  const getTabInfo = (tab: string) => {
    const tabs: Record<string, { label: string; icon: React.ComponentType<any>; color: string }> = {
      dashboard: { label: "Dashboard", icon: Target, color: "bg-purple-100 text-purple-700" },
      learn: { label: "Word Library", icon: BookOpen, color: "bg-green-100 text-green-700" },
      quiz: { label: "Quiz Time", icon: Brain, color: "bg-pink-100 text-pink-700" },
      progress: { label: "My Journey", icon: Trophy, color: "bg-yellow-100 text-yellow-700" },
      adventure: { label: "Adventure", icon: Sword, color: "bg-blue-100 text-blue-700" },
      parent: { label: "Parent Dashboard", icon: Users, color: "bg-purple-100 text-purple-700" }
    };
    return tabs[tab] || { label: tab, icon: Target, color: "bg-gray-100 text-gray-700" };
  };
        ...
        {recentHistory.length > 3 && (
          <div className="mt-2 pt-2 border-t border-gray-100">
            <div className="text-xs text-gray-500 text-center">
              {isExpanded ? 'Showing all recent' : `${recentHistory.length - 3} more location${recentHistory.length - 3 !== 1 ? 's' : ''}`}
            </div>
          </div>
        )}
      </CardContent>
