...
export const AdventureDashboard: React.FC<AdventureDashboardProps> = ({
  ...
  const [wordHero, setWordHero] = useState<WordHero>(adventureService.getWordHero());
  const [wordsNeedingRescue, setWordsNeedingRescue] = useState<WordAdventureStatus[]>([]);
  ...
  const handleGameComplete = (result: RescueGameResult) => {
    try {
      const { updatedWord, hero, newBadges } = adventureService.processRescueResult(result);
  ...
  const updateMissionProgress = (missionType: string) => {
    ...
    hero.daily_mission_progress.completed_count = updatedMissions.filter(m => m.completed).length;
  ...
  const renderGame = () => {
    ...
    const gameResult = (result: Omit<RescueGameResult, 'game_id' | 'word_id'>) => {
    ...
    switch (currentGame.type) {
      ...
      case 'word_match_race':
        return (
          <WordMatchRace
            words={[currentGame.word, ...words.filter(w => w.id !== currentGame.word.id).slice(0, 5)]}
            onGameComplete={gameResult}
            onClose={() => setCurrentGame(null)}
          />
        );
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 text-lg">
            Become a Word Hero and rescue forgotten words from the mystical realms!
          </p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-sm opacity-90 mb-2">Experience Progress</div>
          ...
          <Card className="bg-orange-50 border-orange-200">
            <CardContent className="p-4 text-center">
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <span className="font-bold text-orange-700">Need Attention</span>
              </div>
              <div className="text-2xl font-bold text-orange-600">
                {wordsNeedingRescue.filter(w => w.health >= 30 && w.health < 50).length}
              </div>
            </CardContent>
          </Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {wordsNeedingRescue.slice(0, 6).map((wordStatus) => {
                      const word = words.find(w => w.id.toString() === wordStatus.word_id);
                      if (!word) return null;

                      const PriorityIcon = getPriorityIcon(wordStatus.health);
                      
                      return (
                        <Card key={wordStatus.word_id} className="border-2 border-red-300">
                          <CardContent className="p-4">
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center gap-2">
                                <PriorityIcon className={`w-5 h-5 ${getPriorityColor(wordStatus.health)} ${wordStatus.health < 30 ? 'animate-pulse' : ''}`} />
                                <span className="font-bold">{word.word}</span>
                              </div>
                              <span className={`text-sm font-bold ${getPriorityColor(wordStatus.health)}`}>
                                {wordStatus.health}%
                              </span>
                            </div>
                            ...
                            <Progress 
                              ...
                              className={`h-2 mb-3 ${
                                wordStatus.health >= 50 ? 'bg-green-100' :
                                wordStatus.health >= 30 ? 'bg-orange-100' : 'bg-red-100'
                              }`}
                            />

                            <div className="flex gap-2">
                              <Button
                                onClick={() => handleOpenRescueGame(wordStatus.word_id, 'flashcard_duel')}
                                size="sm"
                              ...
                              <Button
                                onClick={() => handleOpenRescueGame(wordStatus.word_id, 'letter_builder')}
                                size="sm"
              ...
              <Card className="bg-green-50 border-green-200">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-green-600 mb-6">
                    Amazing work, Word Hero! All your vocabulary is healthy and happy.
                  </p>
                  <Button
                    onClick={() => setShowMap(true)}
                  ...
                  <div key={mission.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        <Progress 
                          value={(mission.current_progress / mission.target) * 100} 
                          className="w-32 h-2"
                        />
                    ...
                    <div className="text-right ml-4">
                      {mission.completed ? (
                        ...
                        <div className="text-sm">
                          <div className="font-bold text-blue-600">+{mission.reward_xp} XP</div>
                          <div className="font-bold text-yellow-600">+{mission.reward_coins} Coins</div>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div>
                      ...
                      <Progress value={wordHero.experience % 100} className="h-3" />
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4 pt-4">
                      <div className="text-center p-3 bg-blue-50 rounded-lg">
                        <div className="font-bold text-blue-600 text-xl">{wordHero.experience}</div>
                        <div className="text-sm text-gray-600">Total XP</div>
                      </div>
                      <div className="text-center p-3 bg-yellow-50 rounded-lg">
                        <div className="font-bold text-yellow-600 text-xl">{wordHero.coins}</div>
                ...
                <CardContent>
                  {wordHero.badges.length > 0 ? (
                    <div className="grid grid-cols-2 gap-3">
                      {wordHero.badges.map((badge) => (
                        <div key={badge.id} className="p-3 bg-gray-50 rounded-lg text-center">
                          ...
                          <div className="text-xs text-gray-600">{badge.tier}</div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">Complete rescue missions to earn your first badge!</p>
                    </div>
                  )}
                </CardContent>
              </Card>
