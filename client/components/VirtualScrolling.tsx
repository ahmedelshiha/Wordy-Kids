...
export function VirtualScrolling<T>({
  ...
  const { visibleItems, totalHeight, startIndex, setScrollTop } = useVirtualList({
    items,
    itemHeight,
    containerHeight,
    overscan
...
export const VirtualWordList: React.FC<VirtualWordListProps> = ({
  ...
  const renderWordItem = useCallback((word: WordListItem, index: number) => (
    <div
      className={`
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-gray-900 truncate">{word.word}</h3>
...
export function VirtualGrid<T>({
  ...
  const endRow = Math.min(rowsTotal - 1, Math.ceil((scrollTop + containerHeight) / rowHeight) + 2);
  ...
  const visibleItems = useMemo(() => {
    ...
    return result;
  }, [items, startRow, endRow, columnsCount, itemWidth, itemHeight, gap, rowHeight]);
...
export function useVirtualScrollState<T>(
  ...
  const scrollToIndex = useCallback((index: number) => {
    ...
    setScrollTop(Math.min(targetScrollTop, Math.max(0, items.length * itemHeight - containerHeight)));
