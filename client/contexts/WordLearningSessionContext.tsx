...
const WordLearningSessionContext =
  createContext<WordLearningSessionContextType | null>(null);

export function WordLearningSessionProvider({
  children,
}: {
  ...
  const { sessionData, updateSession, clearSession, isSessionValid } =
    useSessionPersistence(initialSessionData, {
  ...
  const getSessionStats = useCallback(() => {
    ...
    const accuracy =
      totalAttempts > 0
        ? Math.round((sessionData.rememberedWords.length / totalAttempts) * 100)
        : 0;
...
export function useWordLearningSession() {
  ...
  if (!context) {
    throw new Error(
      "useWordLearningSession must be used within a WordLearningSessionProvider",
    );
  }
  return context;
}
