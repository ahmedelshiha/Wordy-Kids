...
switch (e.key) {
  case ' ': handleCardFlip(); break;        // Space - flip card
  case 'p': handlePronounce(); break;       // P - pronounce
  case 'f': handleFavorite(); break;        // F - favorite
  case 'Enter': handleCardFlip(); break;    // Enter - activate
}
```
