...
class RealTimeWordDatabaseService {
  ...
  async initialize() {
    ...
    try {
      ...
      const { wordsDatabase, getAllCategories } = await import(
        "@/data/wordsDatabase"
      );
      ...
      console.log(
        "Real-time word database initialized with",
  ...
  private async checkForChanges() {
    try {
      ...
      if (wordsChanged || categoriesChanged) {
        ...
        console.log(
          "Word database changes detected:",
          eventType,
          `(${newWordCount} words)`,
        );
      }
  ...
  private hasCategoriesChanged(newCategories: string[]): boolean {
    ...
    return (
      ...
      JSON.stringify(this.currentCategories.sort())
    );
  }
