import { wordsDatabase, getWordsByCategory, getRandomWords, Word } from '@/data/wordsDatabase';
...
export const generateQuizQuestions = (
  ...
  category?: string,
  quizType: 'definition' | 'spelling' | 'picture' | 'pronunciation' | 'example' = 'definition'
) => {
  ...
  if (difficulty) {
    selectedWords = selectedWords.filter(word => word.difficulty === difficulty);
  }
...
const generateDefinitionQuestion = (word: Word) => {
  const wrongAnswers = shuffleArray(
    wordsDatabase.filter(w => w.id !== word.id && w.category === word.category)
  ).slice(0, 3);
  
  if (wrongAnswers.length < 3) {
    const additionalWrong = shuffleArray(
      wordsDatabase.filter(w => w.id !== word.id && !wrongAnswers.includes(w))
...
export const generateFillInBlank = (
  ...
  return selectedWords.map(word => {
    ...
    const wordInSentence = sentence.toLowerCase().includes(word.word.toLowerCase());
