...
import { Volume2, Play, Pause, Settings, User, Users, Baby, Star, Heart, Smile, AlertCircle, Loader } from 'lucide-react';
...
export const PronunciationProvider: React.FC<{
  ...
  const [voicePreference, setVoicePreference] = useState<VoiceType>(defaultSettings.voiceType || VOICE_TYPES.WOMAN);
  ...
  const isKidVoice = (name: string, voice: SpeechSynthesisVoice): boolean => {
    const kidIndicators = ['child', 'kid', 'young', 'junior', 'teen', 'boy', 'girl'];
    return kidIndicators.some(indicator => name.includes(indicator));
  };

  const isFemaleVoice = (name: string, voice: SpeechSynthesisVoice): boolean => {
    const femaleIndicators = [
      'female', 'woman', 'girl', 'karen', 'susan', 'samantha', 'victoria', 
      'allison', 'anna', 'emma', 'jenny', 'lisa', 'mary', 'sarah'
    ];
  ...
  const isMaleVoice = (name: string, voice: SpeechSynthesisVoice): boolean => {
    const maleIndicators = [
      'male', 'man', 'boy', 'daniel', 'alex', 'tom', 'fred', 'ralph',
      'david', 'john', 'mike', 'steve', 'paul', 'james'
    ];
    return maleIndicators.some(indicator => name.includes(indicator));
  };

  // Voice selection with quality scoring
  const selectVoiceByPreference = useCallback((voicesList: SpeechSynthesisVoice[], preference: VoiceType) => {
    ...
    if (preferredVoices.length > 0) {
      ...
      selectedVoiceFromCategory = allVoices.find(v => v.default) || allVoices[0] || null;
    }
  ...
  const speak = useCallback(async (text: string, options: {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        utterance.pitch = Math.max(0.1, Math.min(2.0, options.pitch || pitch));
  ...
  }, [isSupported, isVoicesLoaded, selectedVoice, rate, pitch, volume, language, retryCount, onError]);
  ...
  // Convenience methods
  const quickSpeak = useCallback((text: string) => {
    return speak(text, { 
  ...
  const slowSpeak = useCallback((text: string) => {
    return speak(text, { 
  ...
  const phoneticSpeak = useCallback((text: string) => {
    return speak(text, { 
  ...
  useEffect(() => {
    const savedVoiceType = localStorage.getItem('unified-voice-preference') as VoiceType;
  ...
  const contextValue = useMemo(() => ({
    ...
    voices, isVoicesLoaded, voicePreference, selectedVoice, rate, pitch, volume,
    language, isPlaying, currentWord, isSupported, queue,
    speak, stop, quickSpeak, slowSpeak, phoneticSpeak, categorizeVoices
  ]);
    ...
    <div className={`bg-white rounded-xl p-6 shadow-lg ${className}`}>
      ...
      {showAdvanced && (
        <div className="mt-4 text-sm text-gray-600">
          Available voices: {voices.length} | Status: {isVoicesLoaded ? 'Ready' : 'Loading...'}
        </div>
      )}
    </div>
  );
...
export const PronounceableWord: React.FC<{ 
  ...
  const { quickSpeak, slowSpeak, currentWord, isSupported } = usePronunciation();
  ...
  return (
    ...
    >
      {children}
      {showIcon && (
        <Volume2 className={`h-3 w-3 ${isLocalPlaying ? 'animate-pulse' : 'opacity-50'}`} />
      )}
    </button>
  );
};
...
export const PronounceableSentence: React.FC<{ 
  ...
  const handleSpeak = async () => {
    ...
    try {
      await speak(text, {
        rate: rate || undefined,
        onWordHighlight: autoHighlight ? (word) => setHighlightedWord(word) : undefined,
        onEnd: () => {
          setHighlightedWord('');
          setIsLocalPlaying(false);
          onComplete?.();
  ...
  const renderText = () => {
    ...
    return words.map((word, index) => {
      ...
      const cleanHighlighted = highlightedWord.replace(/[^\w]/g, '').toLowerCase();
      
      return (
        <span
          key={index}
          className={cleanWord === cleanHighlighted ? 'bg-yellow-200 px-1 rounded animate-pulse' : ''}
        >
          {word}
        </span>
      );
  ...
  return (
    <div className={`flex items-start gap-3 ${className}`}>
      ...
      {showControls && (
        ...
        >
          {isLocalPlaying ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
        </button>
      )}
    </div>
  );
  ...
  return {
    ...
    audioService: {
      pronounceWord: (word: string, options: any = {}) => {
        ...
        console.warn('Using legacy compatibility layer - please migrate to usePronunciation hook');
      },
