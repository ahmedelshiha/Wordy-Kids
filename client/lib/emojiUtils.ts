 ...
 */
export function getRandomEmoji(category: keyof typeof SAFE_EMOJIS = 'learning'): string {
  const categoryEmojis = SAFE_EMOJIS[category];
  if (!categoryEmojis || categoryEmojis.length === 0) {
    console.warn(`No emojis found for category: ${category}, falling back to learning`);
 ...
 */
export function getRandomEmojis(count: number, category: keyof typeof SAFE_EMOJIS = 'learning'): string[] {
  const result: string[] = [];
  const categoryEmojis = SAFE_EMOJIS[category];
  ...
  for (let i = 0; i < count; i++) {
    result.push(categoryEmojis[Math.floor(Math.random() * categoryEmojis.length)]);
  }
 ...
 */
export function sanitizeEmoji(emoji: string, fallbackCategory: keyof typeof SAFE_EMOJIS = 'learning'): string {
  if (isValidEmoji(emoji)) {
    return emoji;
  }
  
  console.warn(`Corrupted emoji detected: ${emoji}, replacing with safe alternative`);
 ...
 */
export function sanitizeEmojiArray(emojis: string[], fallbackCategory: keyof typeof SAFE_EMOJIS = 'learning'): string[] {
 ...
 */
export function detectCorruptedEmojis(text: string): { hasCorrupted: boolean; corruptedPositions: number[] } {
...
export type EmojiConstant = typeof EMOJI_CONSTANTS[keyof typeof EMOJI_CONSTANTS];
