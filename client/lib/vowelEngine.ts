...
const curve = {
  easy:    { choices: 3, hearts: 4, perQuestionMs: 15000, minLen: 3, maxLen: 5, allowY: false },
  medium:  { choices: 4, hearts: 3, perQuestionMs: 12000, minLen: 3, maxLen: 6, allowY: false },
  hard:    { choices: 5, hearts: 2, perQuestionMs: 9000,  minLen: 3, maxLen: 8, allowY: true  },
} as const;
...
function blankRandomVowel(word: string, allowY = false) {
  ...
  const indices = [...word].map((ch, i) => vowels.includes(ch.toLowerCase()) ? i : -1).filter(i => i >= 0);
  if (indices.length === 0) return null;
  const missingIndex = indices[Math.floor(Math.random() * indices.length)];
  const correct = word[missingIndex].toLowerCase();
  const prompt = word.slice(0, missingIndex) + '_' + word.slice(missingIndex + 1);
  return { prompt, missingIndex, correct };
}

/** Main generator. Replace or pipe your existing logic here. */
export function generateQuestions(opts: GenerateOptions): { questions: VowelQuestion[], hearts: number, perQuestionMs: number } {
  const c = curve[opts.difficulty];
  const allowY = opts.allowYAsVowel ?? c.allowY;

  const filteredWords = opts.words.filter(w => 
