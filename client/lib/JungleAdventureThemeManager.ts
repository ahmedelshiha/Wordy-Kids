...
export const JungleAdventureThemeManager = {
  getTheme(): JungleTheme {
    try {
      ...
      return (['parchment','jungle','canopy','river','sunset'] as const).includes(theme) ? theme : 'parchment';
    } catch {
      return 'parchment';
    }
  },
  ...
  getOverlays(): OverlaySettings {
    try {
      ...
      if (!stored) return { fireflies: true, fog: true, glow: true, ripples: false };
      return { fireflies: true, fog: true, glow: true, ripples: false, ...JSON.parse(stored) };
    } catch {
  ...
  applyOverlays(overlays: OverlaySettings) {
    ...
    const root = document.documentElement;
    root.style.setProperty('--jng-firefly-opacity', overlays.fireflies ? '.9' : '0');
    root.style.setProperty('--jng-fog-opacity', overlays.fog ? '.35' : '0');
    root.style.setProperty('--jng-glow-opacity', overlays.glow ? '.25' : '0');
    root.style.setProperty('--jng-ripples-opacity', overlays.ripples ? '.25' : '0');
  },
