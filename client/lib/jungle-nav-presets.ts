...
export const getPresetById = (presetId: string): JungleNavPreset | undefined => {
  return jungleNavPresets.find(preset => preset.id === presetId);
};

export const getPresetConfig = (presetId: string): Partial<JungleAnimationConfig> | undefined => {
  const preset = getPresetById(presetId);
  return preset?.config;
};

export const getAllPresetProps = (presetId: string): Record<string, any> | undefined => {
  const preset = getPresetById(presetId);
...
export const presetComparison = {
  'calm-learning': { focus: 10, engagement: 3, accessibility: 9, performance: 10 },
  'balanced-adventure': { focus: 7, engagement: 7, accessibility: 7, performance: 8 },
  'playful-adventure': { focus: 4, engagement: 10, accessibility: 6, performance: 6 },
  'accessibility-first': { focus: 8, engagement: 2, accessibility: 10, performance: 10 },
  'bedtime-mode': { focus: 9, engagement: 1, accessibility: 9, performance: 10 },
