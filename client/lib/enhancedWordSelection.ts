...
export class EnhancedWordSelector {
  ...
  static generateSystematicSession(
    ...
    const categoryWords = this.getFilteredCategoryWords(selectedCategory, userHistory);
    ...
    console.log(`Session Strategy: ${sessionStrategy}, Exhaustion: ${exhaustionLevel.toFixed(2)}`);
    ...
    switch (sessionStrategy) {
      case "fresh_exploration":
        selectedWords = this.selectFreshWords(categoryWords, userHistory, userProgress);
        break;
        
      case "mixed_reinforcement":
        selectedWords = this.selectMixedWords(categoryWords, userHistory, userProgress, childStats);
        break;
        
      case "targeted_review":
        selectedWords = this.selectReviewWords(userHistory, userProgress, childStats);
        break;
        
      case "cross_category":
        selectedWords = this.selectCrossCategoryWords(userHistory, userProgress, childStats);
        break;
        
      case "adaptive_difficulty":
        selectedWords = this.selectAdaptiveDifficultyWords(categoryWords, userHistory, childStats);
        break;
    ...
    selectedWords = this.applyFinalOptimizations(selectedWords, userHistory, childStats);
    ...
    // Count word types
    const newWords = selectedWords.filter(w => 
  ...
  private static calculateCategoryExhaustion(
    ...
    const seenWords = categoryWords.filter(w => 
  ...
  private static selectMixedWords(
    ...
    // 60% new words
    const newWords = this.selectFreshWords(availableWords, userHistory, userProgress);
    ...
    const forgottenReviews = this.selectForgottenWordsForReview(userHistory, userProgress);
    const rememberedReviews = this.selectRememberedWordsForReview(userHistory, userProgress);
    
    // Prioritize forgotten words
    selected.push(...forgottenReviews.slice(0, Math.min(reviewSlots, forgottenReviews.length)));
  ...
  private static selectReviewWords(
    ...
    // Prioritize forgotten words
    const forgottenWords = this.selectForgottenWordsForReview(userHistory, userProgress);
    selected.push(...forgottenWords.slice(0, Math.floor(this.SESSION_SIZE * 0.7)));
    
    // Add some remembered words for reinforcement
    const rememberedWords = this.selectRememberedWordsForReview(userHistory, userProgress);
    const remainingSlots = this.SESSION_SIZE - selected.length;
  ...
  private static selectCrossCategoryWords(
    ...
    const categories = ["food", "animals", "objects", "nature", "body", "colors"];
    const wordsPerCategory = Math.floor(this.SESSION_SIZE / categories.length);
    const selected: Word[] = [];
    
    categories.forEach(category => {
      const categoryWords = this.getFilteredCategoryWords(category, userHistory);
      const freshWords = this.selectFreshWords(categoryWords, userHistory, userProgress);
    ...
    if (selected.length < this.SESSION_SIZE) {
      ...
      const remaining = allAvailable.filter(w => 
  ...
  private static selectAdaptiveDifficultyWords(
    ...
    Object.entries(difficultyMix).forEach(([difficulty, ratio]) => {
      const difficultyWords = availableWords.filter(w => w.difficulty === difficulty);
  ...
  private static selectRememberedWordsForReview(
    ...
    rememberedWords.sort((a, b) => {
      ...
      const scoreA = (historyA?.consecutiveCorrect || 0) * (historyA?.averageAccuracy || 0);
      const scoreB = (historyB?.consecutiveCorrect || 0) * (historyB?.averageAccuracy || 0);
  ...
  private static shuffleWithProgression(words: Word[]): Word[] {
    ...
    for (let i = 0; i < maxLength; i++) {
      ...
      if (i < medium.length && result.length < this.SESSION_SIZE) result.push(medium[i]);
      if (i < hard.length && result.length < this.SESSION_SIZE) result.push(hard[i]);
    }
  ...
  static calculateWordCooldown(
    ...
    // Ensure bounds
    baseHours = Math.max(this.MIN_COOLDOWN_HOURS, Math.min(this.MAX_COOLDOWN_HOURS, baseHours));
  ...
  static updateWordHistory(
    ...
    // Update stats
    const newConsecutiveCorrect = wasCorrect ? existing.consecutiveCorrect + 1 : 0;
    ...
    console.log(`Word ${word.word}: Correct=${wasCorrect}, Cooldown=${cooldownHours.toFixed(1)}h, Next eligible: ${new Date(updated.nextEligibleTime).toLocaleString()}`);
   ...
   * Calculate session difficulty
   */
  private static calculateSessionDifficulty(words: Word[]): "easy" | "medium" | "hard" {
