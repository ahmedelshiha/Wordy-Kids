...
class PerformanceProfiler {
  ...
  trackComponentRender(componentName: string, renderTime: number, propsChanged: string[] = []): void {
    ...
    if (renderCount > 100) {
      console.warn(`Excessive re-renders detected: ${componentName} has rendered ${renderCount} times`);
    }
  }
  ...
  trackMemoryUsage(component?: string): void {
    ...
    if (this.memorySnapshots.length > this.maxMetrics) {
      this.memorySnapshots.splice(0, this.memorySnapshots.length - this.maxMetrics);
    }
    ...
    if (recentSnapshots.length >= 10) {
      ...
      if (memoryTrend > 1024 * 1024) { // Growing by more than 1MB
  ...
  trackNetworkRequest(url: string, method: string, duration: number, size: number, status: number): void {
    ...
    if (this.networkMetrics.length > this.maxMetrics) {
      this.networkMetrics.splice(0, this.networkMetrics.length - this.maxMetrics);
    }
  ...
  getPerformanceReport(): {
    ...
    // Network performance
    const avgResponseTime = this.networkMetrics.length > 0
      ? this.networkMetrics.reduce((sum, m) => sum + m.duration, 0) / this.networkMetrics.length
      : 0;

    // Performance score (0-100)
    let performanceScore = 100;
    if (slowestComponents.length > 0 && slowestComponents[0].avgRenderTime > 16) {
      performanceScore -= 20;
    }
    if (memoryTrend === 'up') performanceScore -= 15;
    if (avgResponseTime > 1000) performanceScore -= 15;
    if (this.metrics.some(m => m.name === 'long-task')) performanceScore -= 20;
    ...
    return {
      ...
      summary: {
        slowestComponents,
        memoryUsage: {
          current: recentMemory.length > 0 ? recentMemory[recentMemory.length - 1].used : 0,
          trend: memoryTrend
        },
        networkPerformance: {
          avgResponseTime,
...
export function usePerformanceMonitoring(componentName: string) {
  ...
  const trackCustomMetric = React.useCallback((name: string, value: number, metadata?: Record<string, any>) => {
    profiler.addMetric({
...
export class PerformanceMeasurement {
  ...
  constructor(name: string) {
    ...
    if (duration > 16) {
      console.warn(`Performance warning: ${this.name} took ${duration.toFixed(2)}ms`);
    }
...
export class PerformanceOptimizer {
  static analyzeAndRecommend(): string[] {
    ...
    const slowComponents = report.summary.slowestComponents.filter(c => c.avgRenderTime > 16);
...
if (process.env.NODE_ENV === 'development') {
  ...
  (window as any).getPerformanceRecommendations = () => PerformanceOptimizer.analyzeAndRecommend();
}
