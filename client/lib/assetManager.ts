...
export class AssetManager {
  ...
  static async getAssetPath(requestedPath: string): Promise<string> {
    ...
    console.warn(`⚠️ Asset missing: ${normalizedPath}, using fallback: ${fallback}`)
    return fallback
  }
...
export class AudioManager {
    ...
    const loadPromise = new Promise<HTMLAudioElement>((resolve, reject) => {
      ...
      audio.addEventListener('error', (e) => {
        ...
        if (correctedPath !== AssetManager['fallbackAudio']) {
          ...
          fallbackAudio.addEventListener('error', () => reject(e), { once: true })
          fallbackAudio.load()
        } else {
          reject(e)
        }
  ...
  static async playAnimalSound(animalName: string, volume: number = 0.7): Promise<void> {
...
export class EducationalAudioHelper {
  ...
  static async playUISound(soundType: 'open' | 'close' | 'click' = 'click'): Promise<void> {
    const soundMap = {
