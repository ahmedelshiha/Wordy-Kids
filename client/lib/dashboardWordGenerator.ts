...
export interface DashboardWordSession {
  ...
  sessionInfo: {
    ...
    progressionStage: "easy_focus" | "mixed_easy_medium" | "mixed_medium_hard" | "all_difficulties";
...
export class DashboardWordGenerator {
   ...
   */
  private static determineProgressionStage(wordsCompleted: number): "easy_focus" | "mixed_easy_medium" | "mixed_medium_hard" | "all_difficulties" {
  ...
  private static selectWordsByDifficulty(
    ...
    const candidateWords = wordsDatabase.filter(word => 
      difficulties.includes(word.difficulty) &&
      !rememberedWords.has(word.id) // Exclude already remembered words
    );
    ...
    const selectedForgotten = this.shuffleArray(forgottenCandidates).slice(0, forgottenCount);
    ...
    switch (stage) {
      ...
      case "mixed_easy_medium":
        return {
          ...
          progress: ((wordsCompleted - this.EASY_THRESHOLD) / (this.MEDIUM_THRESHOLD - this.EASY_THRESHOLD)) * 100
        };
