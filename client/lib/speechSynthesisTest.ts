...
export class SpeechSynthesisTest {
  ...
  public static logDiagnostics(result: SpeechSynthesisTestResult): void {
    ...
    console.log(
      "🔊 Voices Available:",
      result.hasVoices ? `✅ (${result.voiceCount})` : "❌",
    );
  ...
  public static getRecommendations(
    result: SpeechSynthesisTestResult,
  ): string[] {
    ...
    if (!result.isSupported) {
      ...
      recommendations.push(
        "Consider using a different browser (Chrome, Firefox, Safari, Edge)",
      );
    ...
    if (!result.testPassed) {
      ...
      if (result.browserInfo.userAgent.includes("Chrome")) {
        recommendations.push(
          "In Chrome: Check chrome://settings/content/sound",
        );
      }
    }
