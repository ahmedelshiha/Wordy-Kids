...
import { WordHistory, SystematicWordSelection } from '@/lib/enhancedWordSelection';
...
export const useSessionPersistence = (
  ...
  const saveToStorage = useCallback((key: string, data: SessionData) => {
    try {
  ...
  const loadFromStorage = useCallback((key: string): SessionData | null => {
    try {
  ...
  const saveSession = useCallback((data: Partial<SessionData>) => {
    const existingData = loadFromStorage(SESSION_STORAGE_KEY) || {} as SessionData;
    ...
    const updatedData: SessionData = {
      ...
      totalTimeSpent: (existingData.totalTimeSpent || 0) + (Date.now() - (existingData.lastSaved || sessionStartTimeRef.current)),
    };

    // Convert Map to Array for storage
    if (data.userWordHistory && data.userWordHistory instanceof Map) {
      updatedData.userWordHistory = Array.from(data.userWordHistory.entries());
    }
...
export type { SessionData, LearningProgress, UseSessionPersistenceOptions, SessionPersistenceReturn };
