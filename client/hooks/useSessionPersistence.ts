...
export function useSessionPersistence(
  ...
  const debouncedSave = useCallback((data: SessionData) => {
  ...
  const clearSession = useCallback(() => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('sessionCleared', { detail: { key } }));
  ...
  useEffect(() => {
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === key && event.newValue) {
        try {
          ...
          if (isValidSessionData(newData, version) && newData.lastUpdated > lastSaveRef.current) {
    ...
    const handleCustomSessionEvent = (event: CustomEvent) => {
      if (event.detail.key === key && event.detail.data.lastUpdated > lastSaveRef.current) {
        setSessionData(event.detail.data);
      }
    };

    window.addEventListener('storage', handleStorageChange);
    window.addEventListener('sessionSaved', handleCustomSessionEvent as EventListener);

    return () => {
      window.removeEventListener('storage', handleStorageChange);
      window.removeEventListener('sessionSaved', handleCustomSessionEvent as EventListener);
    };
  }, [key, version]);
...
// Helper functions
function createInitialSessionData(initialData: Partial<SessionData>): SessionData {
  return {
    currentProfile: null,
    isLoggedIn: false,
    activeTab: 'dashboard',
...
function isValidSessionData(data: any, expectedVersion: string): boolean {
  ...
  const requiredProps = [
    'activeTab', 'selectedCategory', 'learningMode', 
    'rememberedWords', 'forgottenWords', 'lastUpdated'
  ];
  ...
  if (data.version && data.version !== expectedVersion) {
    ...
    if (major !== dataMajor) {
      console.warn(`Incompatible session version: ${data.version}, expected: ${expectedVersion}`);
      return false;
    }
  }
...
export const sessionUtils = {
  ...
  importSession: (file: File, key: string = 'wordy-session'): Promise<boolean> => {
