...
interface ParticleEffect {
  id: string;
  type: "sparkle" | "celebration" | "mastery" | "discovery" | "favorite" | "achievement";
  emoji: string;
...
interface CelebrationAnimation {
  type: "word-mastered" | "achievement" | "level-up" | "category-complete" | "streak";
...
export const useJungleAnimations = () => {
  ...
  const animationsEnabled = animationSettings.enabled && 
  ...
  const createParticles = useCallback((
    ...
    for (let i = 0; i < count; i++) {
      ...
      const distance = (config.spread / 2) * (0.7 + Math.random() * 0.6) * intensity;
    ...
    switch (type) {
      case "word-mastered":
        createParticles("mastery", { 
          position, 
          intensity: intensity === "gentle" ? 0.7 : intensity === "explosive" ? 1.5 : 1 
        });
        break;
      case "achievement":
        createParticles("achievement", { 
          position, 
          intensity: intensity === "gentle" ? 0.8 : intensity === "explosive" ? 1.8 : 1.2 
        });
        break;
      case "level-up":
        createParticles("celebration", { 
          position, 
          intensity: intensity === "gentle" ? 0.9 : intensity === "explosive" ? 2 : 1.3 
        });
        break;
      case "category-complete":
        createParticles("discovery", { 
          position, 
          intensity: intensity === "gentle" ? 0.8 : intensity === "explosive" ? 1.6 : 1.1 
        });
        break;
      case "streak":
        createParticles("sparkle", { 
          position, 
          intensity: intensity === "gentle" ? 0.6 : intensity === "explosive" ? 1.4 : 1 
        });
        break;
    }
  ...
  const animateTransition = useCallback((
    ...
    const targetElement = element || document.querySelector('.main-content') as HTMLElement;
  ...
  const updateAnimationSettings = useCallback((updates: Partial<AnimationSettings>) => {
