...
export const useUserProgress = (): UserStatsData => {
  ...
  useEffect(() => {
    const fetchUserProgress = async () => {
      ...
      try {
        const systematicProgress =
          await goalProgressTracker.fetchSystematicProgress(user.id);
        ...
        const updatedProgress: UserProgressData = {
          wordsLearned:
            ...
            0,
          animalsLearned: getAnimalsLearned(
            systematicProgress.categoriesProgress,
          ),
          totalTimeSpent: Math.round(timeSpent / 60), // Convert to minutes
          currentStreak:
            systematicProgress.currentStreak || journeyProgress.streakDays || 0,
