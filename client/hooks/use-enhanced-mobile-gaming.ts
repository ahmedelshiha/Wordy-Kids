...
export const useEnhancedMobileGaming = (
  options: EnhancedMobileGamingOptions = {},
) => {
  const {
    enableHaptics = true,
    enableGestures = true,
  ...
  useEffect(() => {
    const detectCapabilities = () => {
      ...
      const screenSize =
        screenWidth < 480 ? "small" : screenWidth < 768 ? "medium" : "large";
  ...
  const triggerHaptic = useCallback(
    async (pattern: HapticPattern) => {
      ...
      try {
        ...
        if (
          deviceCapabilities.platform === "iOS" &&
  ...
  const recognizeGesture = useCallback(
    (touches: TouchPoint[], endTouches: TouchPoint[]) => {
  ...
  const getTouchableStyle = useCallback(
    (size?: number) => {
