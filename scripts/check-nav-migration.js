...
async function checkForMissingJungleNav() {
  ...
  try {
    ...
    console.error(`❌ ERROR: Failed to check JungleKidNav usage: ${error.message}`);
    return [];
  }
}
  ...
  for (const path of deprecatedPaths) {
    if (existsSync(path)) {
      violations.push({
        file: path,
        type: 'improper_file',
        message: 'DesktopKidNav.tsx should not exist (should be .backup.tsx or .deprecated.tsx only)'
      });
    }
  }
...
async function main() {
  ...
  console.log('This script ensures no deprecated DesktopKidNav usage exists.\n');
  
  const [importViolations, missingJungleNav, deprecatedFiles] = await Promise.all([
  ...
  hasViolations |= printViolations(missingJungleNav, 'JungleKidNav Usage Check');
  ...
  if (hasViolations) {
    ...
    console.log('3. Ensure deprecated files are properly named (.backup.tsx or .deprecated.tsx)');
