...
class CleanupAutomation {
  ...
  async archiveOldMarkdown() {
    ...
    const thresholdDate = new Date(
      now.setMonth(now.getMonth() - this.config.markdownAgeThreshold),
    );
  ...
  async shouldCompressDirectory(dirPath) {
    try {
      ...
      const ageDays =
        (Date.now() - stats.oldestFile.getTime()) / (1000 * 60 * 60 * 24);
  ...
  async compressDirectory(dirPath) {
    ...
    try {
      ...
      execSync(
        `tar -czf "${archivePath}" -C "${path.dirname(dirPath)}" "${dirName}"`,
      );
  ...
  generateArchiveIndex() {
    ...
    const files = fs
      .readdirSync(docsDir)
      .filter((f) => f.endsWith(".md") && f !== "archive-index.md");
