...
class MaintenanceCleanup {
  ...
  async archiveOldDocs() {
    ...
    this.log(`Would archive docs older than ${cutoffDate.toDateString()}`, 'archive');
  }
  ...
  async compressBackups() {
    ...
    for (const [name, folder] of Object.entries(CONFIG.FOLDERS)) {
      if (fs.existsSync(folder)) {
        ...
        if (stats.sizeMB > CONFIG.LARGE_FILE_SIZE_MB && CONFIG.COMPRESSION_ENABLED) {
          this.log(`${folder} (${stats.sizeMB.toFixed(1)}MB) would be compressed`, 'archive');
