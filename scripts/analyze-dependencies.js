...
class DependencyAnalyzer {
  ...
  isBuiltInOrExcluded(depName) {
    ...
    const excluded = [
      ...
      'vite', 'typescript', 'vitest', // Build tools
  ...
  checkDependencyUsage(depName) {
    try {
      ...
      return { used: true, foundInClient: true, foundInServer: false, importCount: 0 };
    }
  }
  ...
  generateReport() {
    ...
    console.log(`📊 Summary:`);
    console.log(`   - Total dependencies analyzed: ${this.usedDeps.length + this.unusedDeps.length}`);
    ...
    if (this.devOnlyInProd.length > 0) {
      ...
      console.log('');
      console.log('💡 Consider moving these to dependencies if they\'re needed at runtime');
      console.log('');
    }
    ...
    this.usedDeps
      ...
      .slice(0, 10)
      .forEach(dep => {
        const location = dep.foundInClient && dep.foundInServer ? 'client+server' : 
                        dep.foundInClient ? 'client' : 'server';
    ...
    console.log('2. Check if any unused deps are used indirectly (peer dependencies)');
  ...
  saveReportToFile() {
    const reportPath = path.join(process.cwd(), 'backup', 'dependency-analysis-report.json');
    const report = {
