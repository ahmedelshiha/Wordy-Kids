...
function validateResult(test, result) {
  ...
  if (test.expectedStatus && result.status !== test.expectedStatus) {
    return { passed: false, message: `❌ Expected status ${test.expectedStatus}, got ${result.status}` };
  }
    ...
    for (const [key, expected] of Object.entries(test.expected)) {
      ...
      if (expected instanceof RegExp) {
        if (!expected.test(actual)) {
          return { passed: false, message: `❌ ${key} doesn't match pattern ${expected}` };
        }
      } else if (actual !== expected) {
        return { passed: false, message: `❌ ${key} mismatch. Expected: ${expected}, Got: ${actual}` };
      }
    }
  }
  ...
  if (test.expectedHeaders) {
    for (const [header, expected] of Object.entries(test.expectedHeaders)) {
      ...
      if (expected instanceof RegExp) {
        if (!expected.test(actual)) {
          return { passed: false, message: `❌ Header ${header} doesn't match pattern ${expected}` };
        }
      }
    }
  }
