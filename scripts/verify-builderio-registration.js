...
function verifyComponentExists() {
  ...
  if (!existsSync(componentPath)) {
    console.error(`❌ ERROR: ${REQUIRED_COMPONENT} component not found at ${componentPath}`);
    return false;
  }
...
function verifyComponentProps() {
  try {
    const componentContent = readFileSync('client/components/JungleKidNav.tsx', 'utf8');
    ...
    const missingProps = REQUIRED_PROPS.filter(prop => 
      !componentContent.includes(`${prop}?:`) && !componentContent.includes(`${prop}:`)
    );
    
    if (missingProps.length > 0) {
      console.error(`❌ ERROR: Missing required props: ${missingProps.join(', ')}`);
      return false;
    }
    ...
    console.error(`❌ ERROR: Failed to verify component props: ${error.message}`);
    return false;
  }
}
...
async function verifyNoDeprecatedReferences() {
  try {
    ...
    const files = await glob('**/*.{ts,tsx,js,jsx}', { 
      ignore: ['**/node_modules/**', '**/dist/**', '**/*.backup.*', '**/*.deprecated.*'] 
    });
    ...
    for (const file of files) {
      try {
        ...
        // Check for imports
        if (content.includes(`from "@/components/${DEPRECATED_COMPONENT}"`) ||
    ...
    console.error(`❌ ERROR: Failed to verify deprecated references: ${error.message}`);
    return false;
  }
}
...
function verifyBuilderIoConfig() {
  ...
  if (existingConfigs.length === 0) {
    console.log('⚠️  WARNING: No Builder.io config files found. Ensure JungleKidNav is registered in Builder.io dashboard.');
    return true; // Not an error, just a warning
  }
  
  console.log(`✅ SUCCESS: Found Builder.io config files: ${existingConfigs.join(', ')}`);
  return true;
}
