...
class AssetAnalyzer {
  ...
  checkAssetUsage(asset) {
    try {
      ...
      console.warn(
        `⚠️ Could not check usage for ${asset.name}: ${error.message}`,
      );
  ...
  generateReport() {
    ...
    console.log(`📊 Summary:`);
    console.log(
      `   - Total assets: ${this.usedAssets.length + this.unusedAssets.length}`,
    );
    ...
    console.log(
      "2. For duplicates, keep the most optimized format (prefer .webp > .png > .jpg)",
    );
    console.log("3. Update references to use consistent asset naming");
    console.log(
      "4. Consider converting PNG images to WebP for better compression",
    );
  ...
  saveReportToFile() {
    ...
    const report = {
      ...
      usedAssets: this.usedAssets.map((a) => ({
        name: a.name,
        path: a.relativePath,
        size: a.size,
      })),
