...
const checkAssets = () => {
  ...
  if (fs.existsSync(soundsDir)) {
    const soundFiles = fs
      ...
      .filter(
        (file) =>
          file.endsWith(".mp3") &&
          !fs.statSync(path.join(soundsDir, file)).isDirectory(),
      );
  ...
  if (fs.existsSync(soundsUIDir)) {
    const uiSoundFiles = fs
      .readdirSync(soundsUIDir)
  ...
  const requiredAssets = {
    ...
    "Core Images": ["images/Wordy Jungle Adventure Logo.png", "favicon.svg"],
  };
  ...
  const problematicPatterns = [
    {
      pattern: "sounds/owl-hoot.mp3",
      suggestion: "Use sounds/owl.mp3 instead",
    },
    {
      pattern: "sounds/parrot-chirp.mp3",
      suggestion: "Use sounds/RedParot.mp3 instead",
    },
    {
      pattern: "sounds/monkey-chatter.mp3",
      suggestion: "Use sounds/Kapuzineraffe.mp3 instead",
    },
    {
      pattern: "sounds/elephant-trumpet.mp3",
      suggestion: "Use sounds/Elefant.mp3 instead",
    },
    {
      pattern: "sounds/leaf-rustle.mp3",
      suggestion: "Use sounds/jungle-wind.mp3 instead",
    },
    {
      pattern: "images/screenshot-wide.png",
      suggestion: "Use images/Wordy Jungle Adventure Logo.png instead",
    },
  ];
  ...
  if (totalIssues === 0) {
    ...
    console.log(
      "   1. Use AssetManager.getAssetPath() for automatic fallbacks",
    );
    console.log("   2. Create missing files or use suggested alternatives");
    console.log("   3. Run this script again after fixing issues");
    console.log(
      "   4. The app will still work with fallbacks, but some sounds may not be optimal",
    );
  }
  ...
  console.log(
    '   Use: await AssetManager.getAssetPath("/sounds/owl-hoot.mp3")',
  );
