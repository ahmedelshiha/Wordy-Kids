...
const calculateSpacedRepetitionInterval = (accuracy: number, currentInterval: number): number => {
...
const calculateMasteryLevel = (timesCorrect: number, timesReviewed: number, accuracy: number): number => {
  ...
  return Math.min(Math.round(baseScore + experienceScore + consistencyScore), 100);
};

// Helper function to update child stats
const updateChildStats = (childId: string, wordProgress: WordProgress[]): ChildWordStats => {
  const stats: ChildWordStats = {
    childId,
    totalWordsLearned: wordProgress.length,
    wordsRemembered: wordProgress.filter(wp => wp.status === "remembered").length,
    wordsNeedingPractice: wordProgress.filter(wp => wp.status === "needs_practice").length,
    averageAccuracy: wordProgress.length > 0 
      ? Math.round(wordProgress.reduce((sum, wp) => sum + wp.accuracy, 0) / wordProgress.length)
      : 0,
    totalReviewSessions: wordProgress.reduce((sum, wp) => sum + wp.timesReviewed, 0),
    strongestCategories: [],
    weakestCategories: [],
    recentProgress: [],
    masteryByCategory: [],
  ...
  const categoryMap = new Map<string, { total: number; accuracy: number; mastered: number; needsPractice: number }>();
  
  wordProgress.forEach(wp => {
    if (!categoryMap.has(wp.category)) {
      categoryMap.set(wp.category, { total: 0, accuracy: 0, mastered: 0, needsPractice: 0 });
    }
  ...
  stats.masteryByCategory = Array.from(categoryMap.entries()).map(([category, data]) => ({
    category,
  ...
  const sortedCategories = stats.masteryByCategory.sort((a, b) => b.averageAccuracy - a.averageAccuracy);
  stats.strongestCategories = sortedCategories.slice(0, 3).map(cat => cat.category);
  stats.weakestCategories = sortedCategories.slice(-3).reverse().map(cat => cat.category);
...
export const startLearningSession: RequestHandler = async (req, res) => {
  try {
    const { childId, sessionType, category, difficulty }: StartLearningSessionRequest = req.body;
    ...
    const wordsToReview = [
      { id: "1", word: "elephant", category: "Animals", definition: "A large mammal with a trunk", example: "The elephant at the zoo was huge.", lastAccuracy: 85 },
      { id: "2", word: "microscope", category: "Science", definition: "An instrument for viewing very small objects", example: "We used a microscope to look at cells.", lastAccuracy: 70 },
      { id: "3", word: "rainbow", category: "Nature", definition: "An arc of colors in the sky", example: "After the rain, we saw a beautiful rainbow.", lastAccuracy: 95 },
      { id: "4", word: "volcano", category: "Geography", definition: "A mountain that can erupt lava", example: "The volcano erupted with great force.", lastAccuracy: 60 },
      { id: "5", word: "orchestra", category: "Music", definition: "A large group of musicians", example: "The orchestra played a beautiful symphony.", lastAccuracy: 45 },
...
export const recordWordProgress: RequestHandler = async (req, res) => {
  try {
    ...
    if (existingProgress) {
      ...
      const timesCorrect = existingProgress.timesCorrect + (status === "remembered" ? 1 : 0);
      const timesIncorrect = existingProgress.timesIncorrect + (status === "needs_practice" ? 1 : 0);
      const timesReviewed = existingProgress.timesReviewed + 1;
      const accuracy = Math.round((timesCorrect / timesReviewed) * 100);
      const masteryLevel = calculateMasteryLevel(timesCorrect, timesReviewed, accuracy);
      const currentInterval = existingProgress.spacedRepetitionInterval;
      const newInterval = calculateSpacedRepetitionInterval(accuracy, currentInterval);
      ...
      wordProgress = {
        ...
        averageResponseTime: (existingProgress.averageResponseTime + responseTime) / 2,
        learningSessionIds: [...existingProgress.learningSessionIds, sessionId],
        updatedAt: now,
      };
    } else {
      // Create new progress record
      const masteryLevel = calculateMasteryLevel(status === "remembered" ? 1 : 0, 1, status === "remembered" ? 100 : 0);
    ...
    // Update child stats
    const allProgressForChild = Array.from(wordProgressStore.values()).filter(wp => wp.childId === childId);
    ...
    if (updatedStats.totalWordsLearned % 10 === 0 && updatedStats.totalWordsLearned > 0) {
...
export const endLearningSession: RequestHandler = async (req, res) => {
  try {
    ...
    const allProgressForChild = Array.from(wordProgressStore.values()).filter(wp => wp.childId === session.childId);
